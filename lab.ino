...
// WiFi credentials
const char* ssid = "realme11x";
const char* password = "12345678";

// MQTT Broker details - using local Mosquitto server
const char* mqtt_server = "192.168.165.47";  
const int mqtt_port = 1883;
const char* mqtt_topic = "NodeMCU/Sensor/Gautam";
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect(client_id)) {
      ...
      client.publish(mqtt_topic, "MQ135 Gas Sensor Online");
...
float readGasSensor() {
  ...
  float ratio = (3.3 - voltage) * 1000 / voltage;
  float ppm = 116.6021 * pow(ratio/1720, -2.76903);  // Adjust coefficients as per MQ-7 datasheet
